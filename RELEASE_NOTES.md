...
new install — this server now reports `Configured targets: could not be read (FileNotFoundError) — run `vmware-nsx doctor`.` instead of falling silent. A client shown no listing cannot
...
Every CLI command that reaches NSX now passes the same `guard()` as its MCP twin and writes one
audit row under that tool's name (22 commands, `@audited`). Commands that reach nothing remote say so, with a reason (4, `@cli_local`).
